...
- 钢琴键盘：完整 88 键（A0-C8），鼠标或手指多指演奏，支持左右滚动、滚轮，手机端支持双指捏合缩放琴键（带 1x 重置按钮），PC 和手机端都能按住琴键拖动平移键盘（拖动时自动停止当前键发音），键盘自身不显示滚动条。
- 音色：优先使用内置真实钢琴采样（MusyngKite / Salamander Grand Piano），加载完成前用加法合成音色回退。
...
- 完成庆祝：弹完全部音后播放上扬音符音效和飘动音符特效，可配置开关，默认开启。
- 完成评分：全对 5 星，其余按弹错次数降星，与用时无关。
